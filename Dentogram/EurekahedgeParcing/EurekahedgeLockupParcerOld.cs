...
{
    public class EurekahedgeLockupParcerOld
    {
        ...
        */
        ...
        public EurekahedgeLockupParcerOld()
        {
        ...
        private LockupValue Parce(List<EurekahedgeLockupTockenizer.TokenMatch> tokens)
        {
            ...
            {
                result.LockupType = softOrHardMatch.TokenType == EurekahedgeLockupTokenType.Hard ? LockupType.Hard : LockupType.Soft;
            }
            ...
            {
                result.LockupType = tokens.Any(x => x.TokenType == EurekahedgeLockupTokenType.No) ? LockupType.Soft : LockupType.Hard;
            }
            else if (tokens.Any(x => x.TokenType == EurekahedgeLockupTokenType.Yes))
            {
        ...
        {
            double result;
            return double.TryParse(percentMatch.Value.Substring(0, percentMatch.Value.Length - 1), NumberStyles.Float, formatProvider, out result)
                ? result
                : (double?) null;
        }
        ...
        {
            double result;
            if (numberMatch.TokenType == EurekahedgeLockupTokenType.WordNumberValue)
            {
